...
export interface IPlaneRasterizer
{
    ...
    render(planeId: string, textureCache: PlaneTextureCache, canvas: Texture, id: string, width: number, height: number, scale: number, normal: IVector3D, useTexture: boolean, offsetX?: number, offsetY?: number, maxX?: number, maxY?: number, timeSinceStartMs?: number): PlaneBitmapData;
